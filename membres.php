<?php
    ...
    if($_POST)
    {
        ...
        { 
            $nom_avatar = $_POST['pseudo'] . '_' . time() . '_' . rand(1, 999) . '_' . $_FILES['avatar']['name'];
 
            $chemin_photo = RACINE_SITE . 'assets/uploads/img/' . $nom_avatar;
            ...
            $_SESSION['membre']['avatar'] = $nom_avatar;
        } 
        else {
           
            $nom_avatar = 'default.png';
        }
        ...
        if(empty($msg_erreur))
        {
            if(!empty($_POST))
            {
                $resultat2 = $pdo->prepare("UPDATE membre SET id_membre = :id, pseudo = :pseudo, mdp = :mdp, nom = :nom, prenom = :prenom, email = :email, civilite = :civilite, ville = :ville, code_postal = :code_postal, adresse = :adresse, avatar = :avatar WHERE id_membre = :id");
            ...
            $resultat2->bindValue(':adresse', $_POST['adresse'], PDO::PARAM_STR);
            $resultat2-> bindValue(':avatar', $nom_avatar, PDO::PARAM_STR);
            $resultat2->execute();
            }
        }
        if($resultat2 -> execute()) { //si requête = ok
                // 1) on enregistre le fichier photo dans son emplacement définitif
                if(!empty($_FILES['avatar']['name'])) {
                    copy($_FILES['avatar']['tmp_name'], $chemin_photo);
    ...
    $id_membre = (isset($modif)) ? $modif['id_membre'] : '';
        ...
        if(empty($msg)) {
            ...
            if(!empty($_POST['id_produit'])) {
                ...
                $resultat = $pdo -> prepare("REPLACE INTO produit (id_produit, reference, categorie, titre, description, couleur, taille, public, photo, prix, stock, avatar) VALUES (:id_produit, :reference, :categorie, :titre, :description, :couleur, :taille, :public, :photo, :prix, :stock, :avatar)");

                $resultat -> bindValue(':id_produit', $_POST['id_produit'], PDO::PARAM_INT);
            }
            else {
                //enregistre en BDD
                $resultat = $pdo -> prepare("INSERT INTO produit (reference, categorie, titre, description, couleur, taille, public, photo, prix, stock, avatar) VALUES (:reference, :categorie, :titre, :description, :couleur, :taille, :public, :photo, :prix, :stock, :avatar)");
            }
        }
    debug($_FILES);
    debug($_POST);
